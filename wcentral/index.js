...
function WCentralServer() {
    ...
    this.run = function (port) {
        ...
        let cl = [];        /* the socketIo client holder */
        let sensors = [];
        let wsTimeout = null;
        let wsInterval = null;
        ...
        function startWsTimeout() {
            ...
            wsInterval = setInterval(function(){
                if(wsTimeout !== null && wsTimeout < 4000){
                    wsTimeout += 1000;
                }
                else {
                    ...
                    wsTimeout = null;
                    let sensors = [];
                    cl[0].broadcast.emit('broadClient', cleanArray(sensors));
                }
            }, 1000)
        }
        ...
        const WSserver = ws.createServer(function (conn) {
            ...
            console.log("New websocket connection, SENSOR")
            const data = {
                id: null,
                status: 1,
                temp: null,
            ...
            if (conn.headers['device'] === 'weatherSensor') {
                conn.sendText('sensorReady');
            }
            ...
            conn.on("text", function (str) {       
                ...
                str = JSON.parse(str);
                if (conn.OPEN) {
                    data['id'] = parseInt(str['id']);
                    data['temp'] = parseInt(str['temp']).toFixed(2);
                    data['hum'] = parseInt(str['hum']).toFixed(2);

                    sensors[data['id']] = data;

                    console.log(cleanArray(sensors), 'sensors');
                    
                    if (cl.length > 0) {
                       // console.log(sensors, "sensors");
                        //   dbRecord(sensors[str['id']]); 
                        // only one client have to broadcast
                        cl[0].broadcast.emit('broadClient', cleanArray(sensors));
                        startWsTimeout();
                    }
                }
        ...
        }).listen(3001, "192.168.200.49");

        // Initialize the socket server
        const io = socket(server);
        io.on('connection', function (client) {
            console.log(`Client ${client.id} connected`);
            //add new client to clients holder
            cl.push(client);
